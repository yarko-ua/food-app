...
export const getProduct = createAsyncThunk(
	...
	async (productID) => {
		...
		try {
			const product = await fbdb.collection("products").doc(productID).get()
			...
			return {
				id: productID,
				...product.data(),
				createdAt: product.data().createdAt?.toMillis(),
			}
		} catch (error) {
...
export const addProduct = createAsyncThunk(
	...
	async ({ data }, thunkAPI) => {
		...
		if (data && Object.prototype.hasOwnProperty.call(data, "files")) {
			delete data.files
		}
