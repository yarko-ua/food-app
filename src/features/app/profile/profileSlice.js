...
export const getUserFullInfo = createAsyncThunk(
	...
	async (userID, thunkAPI) => {
		...
		try {
			...
			if (!user.exists) throw new Error("User doesn't exists")
...
export const updateProfilePhoto = createAsyncThunk(
	...
	async (_, thunkAPI) => {
		...
		try {
			const uploadPhoto = await thunkAPI.dispatch(
				uploadToStore({
			...
			if (uploadPhoto.payload.length > 0) {
				const updateCurrentUser = await thunkAPI.dispatch(
					updateUser(uploadPhoto.payload[0])
				) // not necessary ??
