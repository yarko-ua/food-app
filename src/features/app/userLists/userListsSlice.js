...
export const getUserLists = createAsyncThunk(
	...
	async (userID, thunkAPI) => {
		...
		const userListsRef = await fbdb
			.collection(`users/${userID || uid}/lists`)
			.get()

		const userLists = !userListsRef.empty
			? userListsRef.docs
					.map((list) => {
...
export const addProductToList = createAsyncThunk(
	...
	async (data, thunkAPI) => {
		...
		const productAdd = await thunkAPI.dispatch(
			addProduct({ data, plain: false })
		)
		...
		try {
			const listDoc = fbdb.doc(
				`users/${user.uid}/lists/${currentList.id}/products/${product.id}`
			)
...
const lists = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(getUserLists.rejected, (state) => {
				state.loading = false
			})
			...
			.addCase(removeProductFromList.rejected, (state) => {
				state.loading = false
			})
			.addCase(removeProductFromList.fulfilled, (state, action) => {
				state.loading = false
				state.currentList.data = state.currentList.data.filter(
